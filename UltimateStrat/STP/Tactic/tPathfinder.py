...
class tPathfinder(TacticBase):
    ...
    def apply(self, info_manager, id_player):
        ...
        player = info_manager.get_player_position(id_player)  # Position du joueur
        target = info_manager.get_ball_position()  # Position de la balle
        ...
        for i in range(1, 6):  # On remplit la liste des obstacle avec la position de chacun (position, id)
            obstacles_list.append((info_manager.get_player_position(i), i))
        ...
        if get_distance(player, target) > 150:  # Tant que la distance entre le robot est la balle > 100mm
            ...
            for obstacle in obstacles_list:  # Boucle pour chaque obstacle dans la liste
                ...
                obstacle_velocity = info_manager.get_speed(obstacle[1])  # Vitesse de l'obstacle obstacle[1]
